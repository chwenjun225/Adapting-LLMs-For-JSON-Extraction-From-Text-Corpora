...
For information on the different arguments, run `python ./Codes/FSDP_QLORA/train_with_fsdp_qlora.py --help`
...
try: 
    ...
    HQQLinear = None 
    print("Failed to import hqq. Please install it using `pip install hqq`")
    pass 

# To add a new model, import the transformers, attention, MLP layers
...
try:
    ...
    print(f"Failed to import wandb. Please install it using `pip install wandb`")
    pass 
...
class Logger:
    def __init__(self, args, log_to="stdout", project_name="fsdp_qlora", entity=None, group=None, name=None, log_after_n_steps=1, rank=0):
...
def update_progress_bar(progress_bar: tqdm, epoch: int, log_loss: float, log_lr: float, rank: int):
    """Update the progress bar with the current `epoch`, `loss`, and `learning_rate`"""
    if rank == 0:
        if log_lr >= 0:
...
def replace_linear(
    ...
    Parameters:
        ...
        linear_replacement (`torch.nn.Module`):
            ...
            If other arguments need to be passed, use a `lambda` function.
        skip_modules (`List[str]`, *optional*, defaults to `lm_head`):
            List of module names not to convert. Defaults to `lm_head`. 
    """
...
def load_and_quantize(
    ...
    """
    Load `value` tensor in to submodule `module`, optionally skipping `skip_names` and converting to `dtype`.
    Quantizes `Paramas4bit` on `device` then places on "cpu" if `to_cpu=True` or "meta" if `to_meta=True`.
    """
    ...
    try:
        if quant_method == 'bnb':
            ...
            if isinstance(param, Params4bit):
                ...
                # FSDP only syncs parameters and buffers, so the `quant_state` is not copied. This
                # workaround quantizes `Params4bit` to initialize the `quant_state` on all ranks,
                # then replaces `Params4bit`'s data with a meta tensor to free memory on non-rank 0. 
                if is_dora:
...
# DATASET + DATALOADER (modified from llama recipes)
...
def fsdp_main(local_rank: int, world_size: int, args: Dict):
    ...
    # Start logging
    logger = Logger(args, log_to=args["log_to"], project_name=args["project_name"], entity=args["entity"], group=args["group"], name=args["name"], rank=rank)

    # Timing stuff
    init_start_event = torch.cuda.Event(enable_timing=True)
    init_end_event = torch.cuda.Event(enable_timing=True)

    # Model precision, QLoRA compute precision, and FSDP mixed precision policy. 
    ...
    if rank == 0 or args["verbose"]: print(f'>>>>>>>>>> Create model at: {rank} ...<<<<<<<<<<')
    if args["train_type"] in ["full", "lora", "custom_lora"]:
        if (args["low_memory"] and rank == 0) or (not args["low_memory"]):
            model = AutoModelForCausalLM.from_pretrained(args["model_name"], use_cache=False, torch_dtype=torch_dtype, _attn_implementation=attn_impl)
        ...
        # and then places each layer on CPU or meta of using `low_memory` to minimize GPU memory usage 
        ...
        if rank == 0 and args["verbose"]:
            print(f">>>>>>>>>> Loading model weights in {time.time() - start:.3f} seconds ...<<<<<<<<<<")
        # Cleaned any extra memory usage from parallel loading 
        torch.cuda.empty_cache()
    if rank == 0 or args["verbose"]:
        print('>'*10 + f' Rank {rank}: Model created: {torch.cuda.memory_reserved(local_rank)/2**30:.3f} GiB ...<<<<<<<<<<')
    ...
    if args["train_type"] in ["lora", "qlora"]:
        ...
        if rank==0:
            ...
            # And then `setup_quantized_peft_meta_for_training` set `quant_state.to` back to normal 
            setup_quantized_peft_meta_for_training(model)

    elif args["train_type"] in ["custom_qlora", "custom_lora", "hqq_lora", "hqq_dora", "bnb_dora"]:
        if args["train_type"] == "hqq_dora":
            print('>'*10 + f' Using HQQ_DORA, rank: {rank} ...<<<<<<<<<<')
            lora_cls = HQQDORA 
        elif args["train_type"] == "bnb_dora":
            print('>'*10 + f' Using BNB_DORA, rank: {rank} ...<<<<<<<<<<')
            lora_cls = BNBDORA
        else:
            print('>'*10 + f' Using LORA, rank: {rank} ...<<<<<<<<<<')
            lora_cls = LORA
        ...
        for n, p in model.named_parameters():
            if any([lora_name in n for lora_name in ['lora_AB', 'lora_A', 'lora_B', 'magnitude']]):
                ...
                if args["verbose"]:
                    print(f'>>>>>>>>>> Trainable LORA layer {n} ...<<<<<<<<<<')
            else:
        ...
        for n, p in model.named_parameters():
            if any([layer_name in n for layer_name in new_layer_names]):
                p.requires_grad = True
                if args["verbose"]:
                    print(f'>>>>>>>>>> Trainable Llama-Pro layer {n} ...<<<<<<<<<<')
            else:
                p.requires_grad = False
    ...
    # Wrap model with llama-recipes or custom LoRA policy 
    ...
    if rank == 0 or args["verbose"]:
        print(f">>>>>>>>>> Wrapping model w/ FSDP: {rank} ...<<<<<<<<<<")
    ...
    if args["use_gradient_checkpointing"]:
        ...
        if rank == 0 or args["verbose"]:
            print(f">>>>>>>>>> Applying activation checkpointing, rank: {rank} ...<<<<<<<<<<")
        apply_activation_checkpointing(
            model, checkpoint_wrapper_fn=non_reentrant_wrapper, check_fn=check_fn
        )
    if args["use_activation_cpu_offload"]:
        if rank == 0 or args["verbose"]:
            print(f">>>>>>>>>> Applying activation offloading, rank: {rank} ...<<<<<<<<<<")
    ...
    if rank == 0:
        print('>'*10 + f' Total training steps: {num_training_steps} ...<<<<<<<<<<')
    memory_stats = []
    ...
    with profiling_context(args, rank=rank) as prof:
        for epoch in range(args['num_epochs']):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                # Delete the output so more memory free up before the next forward pass 
                output = None 
                loss = None 
                ...
                if args["max_steps"] > 0 and batch_idx > args["max_steps"]:
                    if rank == 0:
                        print(f'>>>>>>>>>> Max steps reached, skipping rest of epoch ...<<<<<<<<<<')
                    break 

            # Print + log peak memory usage for the verbose whole fourth step of training
    ...
    torch.cuda.synchronize()
    if rank == 0:
        print(f'>>>>>>>>>> CUDA event elapsed time: {time_taken} seconds ...<<<<<<<<<<')
    ...
    # None param/buffer types are not saved with FSDP.
    ...
    if args["save_model"]:
        ...
        if args["train_type"] in ["custom_lora", "custom_qlora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]:
            ...
            if rank == 0:
                print(f'>>>>>>>>>> Saving train LoRA weights ...<<<<<<<<<<')
                save_file(cpu_state_dict, os.path.join(args["output_dir"], "model_state_dict.safetensors"))
                print(f'>>>>>>>>>> DONE at rank: {rank} ...<<<<<<<<<<')
        else:
            with FSDP.state_dict_type(model, StateDictType.FULL_STATE_DICT, save_policy):
                cpu_state_dict = model.state_dict()
                if rank == 0:
                    print('>>>>>>>>>> Saving full model weight ...<<<<<<<<<<')
                    save_file(cpu_state_dict, os.path.join(args["output_dir"], "model_state_dict.safetensors"))
                    print(f'>>>>>>>>>> DONE at rank: {rank} ...<<<<<<<<<<')
    # Stop other processes ending while model saving - probably not needed?
    dist.barrier()
    # Clean up the memory 
...
# Main entry point, validate and package arguments, then call `fsdp_main`. 
# Scripts importing `train.py` as an import can invoke this function 
# (`invoking main()` directly leads to call_parse() overwriting arguments).
def fsdp_qlora(
    ...
    batch_size: int = 5, # Batch size per GPU. Effective BS = batch_size * world_size * gradient_accumulation_steps
    ...
    save_model: bool = True, # Save the resulting model
    output_dir: str = "results", # Output directory to save the final model to
    ...
    Args:
        world_size: 
            Number of GPUs to use. -1 = all available GPUs
        train_type: 
            "full", "lora", "qlora", or "custom_qlora"
        llama_pro_path: 
            Path to the quantized llama pro model
        batch_size: 
            Batch size per GPU. Effective BS = batch_size * world_size * gradient_accumulation_steps
        context_length: 
            Max length of input sequence (in tokens)
        gradient_accumulation_steps:
            How many steps to accumulate gradients over (increases effective batch size)
        num_epochs: 
            How many epochs of training to do
        dataset: 
            alpaca, alpaca_sample (for a 128-sample test) or "dummy" for 16 long dummy samples
        dataset_samples: 
            Number of samples in an epoch if using "alpaca_sample" or "dummy" dataset
        sharding_strategy: 
            Sharding strategy for FSDP
        use_gradient_checkpointing: 
            Use FSDP's activation checkpointing
        reentrant_checkpointing: 
            Use re-entrant autograd activation checkpointing. Setting to True can use less GPU memory with BNB QLoRA
        use_cpu_offload: 
            Use FSDP's CPU offloading
        use_activation_cpu_offload: 
            Use FSDP's activation CPU offloading
        low_memory: 
            Load one copy of the model into CPU memory before sharding with FSDP. For QLoRA, quantizes each layer individually on GPU before placing on CPU.
        no_sync: 
            Prevent gradient sync until update step. Likely uses more memory. Required for `use_cpu_offload` and `gradient_accumulation_steps > 1`
        precision: 
            Training precision, `autocast` precisions use mixed precision
        model_name: 
            Which model to train - e.g. "TinyLlama/TinyLlama-1.1B-Chat-v1.0",
        save_model: 
            Save the resulting model
        output_dir: 
            Output directory to save the final model to
        lora_rank: 
            LoRA rank for lora/qlora
        lora_alpha: 
            LoRA alpha for lora/qlora
        lora_dropout: 
            LoRA dropout for lora/qlora
        lora_target_modules: 
            If 'default', uses peft defaults. Use 'all' for our best guess for Llama models
        verbose: 
            Whether to print extra info for debugging
        lr: 
            Learning rate
        ...
        wd: 
            Weight decay
        profile_memory: 
            Profile memory usage for the first few batches. Keep false for training. May increase memory usage.
        optimizer: 
            Optimizer, Pytorch 2.4 nightly adds CPU fused Adam/AdamW which should improve offload training speed.
        lr_scheduler: 
            Learning Rate Scheduler, linear and cosine warm up for 10% of training steps
        loading_workers: 
            Number of layers to load and quantize in parallel per GPU. Default of -1 uses heuristics to set worker count
        log_to: 
            Where to log output
        master_addr: 
            For distributed training
        master_port: 
            For distributed training, must be the same for all processes
        seed: 
            Random seed
        project_name: 
            For wandb logging
...
def main(
    ...
    llama_pro_path: str = None, # Path to the quantized llama pro model
    batch_size: int = 1, # Batch size per GPU. `Effective_Batch_Size = batch_size * world_size * gradient_accumulation_steps`
    context_length: int = 512, # Max length of input sequence (in tokens)
    gradient_accumulation_steps: int = 1, # How many steps to accumulate gradients over (increases effective batch size)
    ...
    master_addr: str = "localhost", # For distributed training
    master_port: str = "12356", # For distributed training, must be the same for all processes
    seed: int = 42, # Random seed
    project_name: str = "chwenjun225_fsdp_qlora", # For wandb logging
    name: str = None, # For wandb logging
    group: str = None, # For wandb logging
    entity: str = None, # For wandb logging
    n_bits: int = 4, # Passed to hqq
